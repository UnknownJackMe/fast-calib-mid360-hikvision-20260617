...
int main(int argc, char **argv)
{
  ...
  if (qr_center_cloud->size() != 4)
  {
    ...
    cv::imwrite(params.output_path + "/qr_detect.png", qrDetectPtr->imageCopy_);
    rclcpp::shutdown();
    return 2;
  }
  ...
  {
    RCLCPP_ERROR(node->get_logger(), "Expected 4 LiDAR circle centers, got %zu", lidar_center_cloud->size());
    cv::imwrite(params.output_path + "/qr_detect.png", qrDetectPtr->imageCopy_);
    rclcpp::shutdown();
    return 3;
  }
